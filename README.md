...
**游戏设定：** 现代都市，丧尸爆发第1天。你是一名困在地铁站避难所里的幸存者。论坛小道消息传来：城市另一端的实验室有血清。你的目标——活着穿越整座城市，到达实验室最深处。
...
你只需要做一件事：**用自然语言告诉 AI 你（或你控制的多名角色）想做什么。**
...
你不需要记住任何规则——AI 会处理所有检定、计算和判定。你只需要做决定（单角色或多角色都可）。
...
这会在 `tools/dice_pool.md` 生成一个包含73个骰子的列表。游戏开始时把内容发给AI DM。用完了再跑一次补充。
...
**Q：AI 可以公平地投骰吗？**
A：本系统使用**骰池制**——你提前用骰子工具投一批骰子（d100×50个、d10×15个等），以列表形式发给AI。AI严格按顺序消耗，不能跳过或挑选。这样骰子结果由你生成（客观公正），AI又不需要每次检定都停下来等你（流畅不卡顿）。详见 `rules/03_骰池协议.md`。

**Q：一次会话要多久？**
A：取决于你。可以玩20分钟做一次探索，也可以玩几个小时推进一整章。存档系统让你随时中断和继续。

**Q：可以多人游戏吗？**
A：可以。支持多人共用一个会话，也支持单人控制多角色。推荐使用 `dm_guide/原始输入协议_多人多角色.md` 的输入结构，便于 DM 统一裁决。
