...
4. 每消耗一个骰子，立即从骰池文件（`tools/dice_pool.md`）删除对应数字
5. 用完后提醒玩家补充
...
- **存档 = 剪切**：运行态文件被移到归档目录，原位置不保留。
- **读档 = 剪切**：归档文件被移回运行态目录，归档被消费。
- **不做备份**：数据始终只在一个地方。
- **单槽位**：同一战役只保留最新一份存档。
- 工具位置：`tools/save_manager.py`

### 6.2 运行态判断与决策

DM 在每次会话开始时**必须先读取 `characters/active/` 目录**，检查是否有角色卡（排除"示例角色*"）：

| 运行态 | 玩家意图 | DM操作 |
|--------|----------|--------|
| 有角色卡（有运行态） | "继续游戏" | **直接继续**，不执行任何存档/读档命令 |
| 有角色卡 | "读取其它存档" | 先 `archive` 当前局 → 确认角色已清空 → 再 `restore` 目标存档 |
| 有角色卡 | "开始新游戏" | 先 `archive` 当前局 → 确认角色已清空 → 再引导角色创建 |
| 无角色卡（无运行态） | "继续游戏" | 执行 `python tools/save_manager.py restore -c [campaign_id]` |
| 无角色卡 | "开始新游戏" | 直接引导角色创建 |
...
存档文件（`saves/save_YYYYMMDD_HHMMSS_[主角色串]_[AI超简评].md`）包含：
...
读档**仅在 `characters/active/` 无角色卡时需要**。

如果有角色卡但玩家想切换到其它存档，必须先 `archive` 当前局。

```
无角色卡 → 读档步骤：
1. python tools/save_manager.py restore -c [campaign_id]（剪切回运行态）
2. 读取存档文件
...
有角色卡但要切换 → 先存后读：
1. python tools/save_manager.py archive -c [当前campaign_id] --ai-blip "[摘要]"
...
- **内容**：场景描述、对话、事件、玩家选择
- **风格**：类似小说的叙事风格
- **文件名**：`session_YYYYMMDD_XX_[队伍标签].md`（队伍标签可选）

#### ⚔️ 战斗日志 (`logs/combat/`)
- **用途**：详细的战斗过程记录
- **内容**：每轮行动、检定结果、伤害计算、状态变化
- **风格**：表格化、数据化
- **文件名**：`combat_YYYYMMDD_XX_[队伍标签].md`（队伍标签可选）

#### 🔍 探索日志 (`logs/exploration/`)
- **用途**：探索发现和物资记录
- **内容**：探索的区域、发现的物品、遇到的NPC、获得的信息
- **风格**：清单式记录
- **文件名**：`exploration_YYYYMMDD_XX_[队伍标签].md`（队伍标签可选）

#### ⚙️ 系统日志 (`logs/system/`)
- **用途**：所有数值变动和检定记录
- **内容**：投骰结果、HP/SP/EP变化、经验获取、物品增减
- **风格**：日志格式，每行一条记录
- **文件名**：`system_YYYYMMDD_[队伍标签].md`（队伍标签可选）
